 ...
 * Reads backend/src/data/stockholmStationsSeed.json (or legacy .ts seed).
...
const jsonSeed = join(root, '..', 'backend', 'src', 'data', 'stockholmStationsSeed.json');
const tsSeedPaths = [
...
if (existsSync(jsonSeed)) {
  const all = JSON.parse(readFileSync(jsonSeed, 'utf8'));
  stations = all.filter((s) => ['METRO', 'TRAIN', 'FERRY'].includes(s.stop_type));
} else {
  const seedPath = tsSeedPaths.find((p) => existsSync(p));
  if (!seedPath) {
